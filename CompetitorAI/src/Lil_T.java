...
public class Lil_T implements AI {
    ...
    private void moveCleaners(AIGameState state) {
        Node wizardLoc = state.getMyWizard().getLocation();

        for(Cleaner cleaner : state.getMyCleaners()) {
            //every cleaner should find wizard
            for(Blocker eb : state.getEnemyBlockers()){
                if(eb.isAdjacent(cleaner) && eb.isBlocking()){
                    cleaner.sweep(eb);
                    //cleaner.move(cleaner.getDirection(cleaner.getLocation(), eb.getLocation(), pathWeight));
                }
                else{
                    cleaner.move(cleaner.getDirection(state.getPath(cleaner.getLocation(), eb.getLocation(), pathWeight)));
                }
            }
        }
